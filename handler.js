  ...
  async participantsUpdate({ jid, participants, action }) {
    ...
    switch (action) {
      ...
      case 'remove':
        ...
        break
      case 'promote':
        text = (chat.sPromote || this.spromote || conn.spromote || '@user ciee jadi admin<3')
      case 'demote':
        if (!text) text = (chat.sDemote || this.sdemote || conn.sdemote || '@user mamfuss di unadmin:v')
...
global.dfail = (type, m, conn) => {
  let msg = {
    rowner: '_Lu mau ngapain?_',
    owner: '_Lu mau ngapain?_',
    mods: '_Lu bukan moderator mana bisa<3_',
    premium: '_Lu bukan user premium, daftar dulu sama ownerku ketik #owner_',
    group: '_Di grup di sini mana bisa_',
