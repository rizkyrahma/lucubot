        ...
        if (chat.welcome) {
          ...
          for (let user of participants) {
            ...
            try {
              ...
              await this.send2ButtonLoc(jid, await (await fetch(fla + (action === 'add' ? 'welcome' : 'goodbye'))).buffer(), text, watermark, action === 'add' ? 'Welcome' : 'Goodbye', 'rasel', action === 'add' ? 'Off Welcome' : 'Off Goodbye', '.0 w', null, {
                contextInfo: {
                  mentionedJid: [user]
                }
              }
