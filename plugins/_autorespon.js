...
`.trim(), watermark, 'Pemilik Bot', ',owner', 'Cara memasukkan bot', '.panduanadd', m)
    ...
    if (new Date() * 1 - setting.status > 1000) {
        ...
        await this.setStatus(`Aktif selama ${uptime} | Mode: ${global.opts['self'] ? 'Private' : setting.groupOnly ? 'Hanya Grup' : 'Publik'} |  Database : No Lucu 乂⁩ Bot © by @rasel.ganz ×͜×`).catch(_ => _) // disable jika kamu tidak suka dengan ini
